...
#Establish data connection using PAS System Environment Variables
...
fetch_day <- function(day) 
{
    #Pull Current Season Days
    scrape(start = day, end = day, suffix = "inning/inning_all.xml", connect = my_db$con)
}

start <- as.Date("06-01-18",format="%d-%m-%y")
end   <- as.Date("06-30-18",format="%d-%m-%y")

dates <- seq(start, end, by=1)

for (d in dates) {
    print(paste0("Scraping MLB for ",format(theDate,"%d-%m-%y")))
    fetch_day(theDate)
}
...
week1DF <- scrape(start = "2018-03-29", end = "2016-04-07", suffix = "inning/inning_all.xml", connect = my_db$con)
rm(week1DF)
#week2DF <- scrape(start = "2018-04-08", end = "2016-04-14", suffix = "inning/inning_all.xml", connect = my_db$con)
#rm(week2DF)
#week3DF <- scrape(start = "2018-04-15", end = "2016-04-21", suffix = "inning/inning_all.xml", connect = my_db$con)
#rm(week3DF)
#week4DF <- scrape(start = "2018-04-22", end = "2016-04-28", suffix = "inning/inning_all.xml", connect = my_db$con)
#rm(week4DF)
...
#         startdate = Today - TodayYearDay - (365 - PostSeasonEndYearDay) - 120
#         splitSeasonsPreviousDays = 0
#     } else {
...
#     startdate = Today - (TodayYearDay - PostSeasonEndYearDay) - 120
...
# 
# #Pull Current Season Days
# scrape(start = startdate, enddate, suffix = "inning/inning_all.xml", connect = my_db$con)
# 
